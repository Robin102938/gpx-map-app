...
FOOTER_PAD_BOT = 120 # zusätzlicher Unterkante
...
if st.button("Poster erzeugen") and gpx_file and event_name and runner and duration:
        ...
        for seg in tr.segments:
            for pt in seg.points:
                if pt.time and pt.elevation is not None:
                    raw.append((pt.longitude, pt.latitude, pt.elevation, pt.time))
    if len(raw) < 2:
    ...
    be = tmp_draw.textbbox((0, 0), ev, font=f_event)
    bi = tmp_draw.textbbox((0, 0), info, font=f_info)
    bm = tmp_draw.textbbox((0, 0), bib, font=f_meta)
