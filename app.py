...
FOOTER_PAD_BOT = 120 # extra untere Padding, um letzte Zeile zu schützen
...
if st.button("Poster erzeugen") and gpx_file and event_name and runner and duration:
    ...
    if len(pts)>MAX_PTS_DISPLAY:
        step=len(pts)//MAX_PTS_DISPLAY+1; pts=pts[::step]
    # 4) Karte rendern
    ...
    try: map_img=m.render(zoom=None)
    except:
        map_img=Image.new("RGB",(MAP_W,MAP_H),"white")
        df=ImageDraw.Draw(map_img); df.line(pts,fill="black",width=10)
